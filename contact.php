...
function mail_message($data_array, $template_file, $deadline_str, $myname, $myemail) {
   ...
   $email_message = str_replace("#NAME#", $data_array['name'], $myname, $email_message);
   $email_message = str_replace("#EMAIL#", $data_array['email'], $myemail, $email_message);
...
if (!($customer_name && $customer_email && $_GET['whoami'] 
   ...
   $url = "http://".$_SERVER['HTTP_HOST']."/php - intro course/contact_form.php?".$query_string."&error=1";
   header("Location: ".$url);
...
<?
include($_SERVER['DOCUMENT_ROOT']."/php - intro course/template_bottom.inc");
?>